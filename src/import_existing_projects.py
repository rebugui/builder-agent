#!/usr/bin/env python3
"""
Builder Agent - Import Existing Projects

Project 폴더의 기존 프로젝트들을 데이터베이스로 가져옵니다.
"""
...
def import_existing_projects():
    ...
    projects = [
        {
            ...
            "status": "Done",
            "deployed_url": "https://github.com/yourusername/encoder"  # 실제 URL로 변경 필요
        },
        {
    ...
    for project in projects:
        ...
        try:
            ...
            # 배포 URL 업데이트 (있는 경우)
            if project.get("deployed_url"):
                planner.update_project_url(project_id, project["deployed_url"])

            print(f"✅ 추가 완료: {name} (ID: {project_id})")
            added_count += 1
